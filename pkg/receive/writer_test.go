	...
	for testName, testData := range tests {
		t.Run(testName, func(t *testing.T) {
			...
			for idx, req := range testData.reqs {
				err = w.Write(context.Background(), DefaultTenant, newProtobufWriteRequest(req.Timeseries), true)
...
func benchmarkWriter(b *testing.B, labelsNum int, seriesNum int, generateHistograms bool) {
	...
	b.Run("without interning", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			testutil.Ok(b, w.Write(ctx, "foo", newProtobufWriteRequest(wreq.Timeseries), true))
		}
	})
