	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, ts := range tc.wreq.Timeseries {
				lset := labelpb.LabelpbLabelsToPromLabels(ts.Labels)
...
func TestDistributeSeries(t *testing.T) {
	...
	_, remote, err := h.distributeTimeseriesToReplicas(
		...
		[]*prompb.TimeSeries{
			{
				Labels: labelpb.PromLabelsToLabelpbLabels(labels.FromStrings("a", "b", tenantIDLabelName, "bar")),
			},
			{
				Labels: labelpb.PromLabelsToLabelpbLabels(labels.FromStrings("b", "a", tenantIDLabelName, "boo")),
			},
		},
	)
	require.NoError(t, err)
	...
	require.Equal(t, 1, labelpb.LabelpbLabelsToPromLabels(remote[endpointReplica{endpoint: endpoint, replica: 0}]["bar"].timeSeries[0].Labels).Len())
	require.Equal(t, 1, labelpb.LabelpbLabelsToPromLabels(remote[endpointReplica{endpoint: endpoint, replica: 0}]["boo"].timeSeries[0].Labels).Len())
