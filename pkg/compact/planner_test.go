...
func TestPlanners_Plan_Compatibility(t *testing.T) {
	...
	tsdbComp, err := tsdb.NewLeveledCompactor(context.Background(), nil, nil, ranges, nil, nil, true)
	testutil.Ok(t, err)
	...
	for _, c := range []struct {
		...
		},
		{
			...
			metas: []*metadata.Meta{{BlockMeta: tsdb.BlockMeta{Version: 1, ULID: ulid.MustNew(1, nil), MinTime: 0, MaxTime: 540, Stats: tsdb.BlockStats{
				NumSeries:     10,
				NumTombstones: 3,
			}}}},
		},
		...
		//                                                       |--------------|
		{
			...
			metas: []*metadata.Meta{
				...
				{BlockMeta: tsdb.BlockMeta{Version: 1, ULID: ulid.MustNew(3, nil), MinTime: 17, MaxTime: 35}},
		...
		t.Run(c.name, func(t *testing.T) {
			...
			t.Run("tsdbPlannerAdapter", func(t *testing.T) {
				...
				testutil.Ok(t, err)
				if len(c.expected) == 0 {
					testutil.Equals(t, len(plan), 0)
				} else {
					testutil.Equals(t, c.expected[0], plan[0])
				}
			})
